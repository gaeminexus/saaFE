...
### Combos (selects) alimentados desde una tabla — búsqueda mínima obligatoria
Siempre que una pantalla incluya un combo/`select` y ese combo se llene desde una tabla **distinta a la de rubros**, la búsqueda del combo debe permitir filtrar por **al menos dos campos**. La **única excepción** es cuando la tabla tiene exactamente 3 campos — `id`, `nombre` y `estado` —; en ese caso basta con buscar solo por `nombre`. En cualquier otro caso siempre debe buscarse por `nombre` más un segundo campo identificatorio propio de esa tabla (p. ej. `nombre` y cuenta contable, `nombre` y cédula, y así sucesivamente según los campos disponibles).
