...
const ESTADO_GENERADO = 3;

/**
 * Cheques recién generados (estado 3) que todavía no se imprimieron.
 * Antes tenía filas hardcodeadas; ahora consulta GET /dtch/listar.
 */
@Component({
  selector: 'app-cheques-generados',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './cheques-generados.component.html',
  styleUrls: ['./cheques-generados.component.scss'],
})
export class ChequesGeneradosComponent implements OnInit, AfterViewChecked {
  ...
  constructor() {
    ...
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case 'fecha': {
          const fecha = item.fechaUso ?? item.fechaImpresion ?? item.fechaEntrega ?? null;
