...
interface ChequeGeneradoRow {
  numero: string;
  beneficiario: string;
  banco: string;
  cuenta: string;
  valor: number;
  fechaGenerado: string; // ISO
}
...
export class ChequesGeneradosComponent {
  ...
  rows = signal<ChequeGeneradoRow[]>([
    {
      ...
      cuenta: '100-001',
      valor: 520.4,
      fechaGenerado: new Date().toISOString().slice(0, 10),
    },
    {
      numero: '000201',
  ...
  filtered = computed(() => {
    ...
    return this.rows().filter((r) => {
      const inRange =
        (!fi || r.fechaGenerado >= fi.toISOString().slice(0, 10)) &&
        (!ff || r.fechaGenerado <= ff.toISOString().slice(0, 10));
