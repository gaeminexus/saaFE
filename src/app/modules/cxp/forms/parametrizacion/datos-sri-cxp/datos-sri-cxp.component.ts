...
import { ListadoSriCxpService } from '../../../service/listado-sri-cxp.service';
import { DetalleSriCxpService } from '../../../service/detalle-sri-cxp.service';
...
@Component({
  ...
  templateUrl: './datos-sri-cxp.component.html',
  styleUrls: ['./datos-sri-cxp.component.scss']
})
export class DatosSriCxpComponent implements OnInit {
  private fb = inject(FormBuilder);
  private dialog = inject(MatDialog);
  private snackBar = inject(MatSnackBar);
  private listadoService = inject(ListadoSriCxpService);
  private detalleService = inject(DetalleSriCxpService);
  ...
  // Listados SRI CXP (padre)
  listados = signal<ListadoSriCxp[]>([]);
  listadoSeleccionado = signal<ListadoSriCxp | null>(null);
  ...
  // Detalles SRI CXP (hijo)
  detalles = signal<DetalleSriCxp[]>([]);
  detalleEditando = signal<DetalleSriCxp | null>(null);
  dataSourceDetalles = new MatTableDataSource<DetalleSriCxp>([]);
  ...
  cargarListados(): void {
    ...
    dbEstado.asignaUnCampoSinTrunc(TipoDatos.LONG, 'estado', '1', TipoComandosBusqueda.IGUAL);
  ...
  seleccionarListado(listado: ListadoSriCxp): void {
  ...
  eliminarListado(listado: ListadoSriCxp): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed && listado.id) {
        this.listadoService.delete(listado.id).subscribe({
          ...
          error: () => this.mostrarError('Error al eliminar listado'),
        });
      }
    });
  }
  ...
  cancelarListado(): void {
    if (this.modoListado() === 'nuevo') {
      ...
      if (listado) this.formListado.patchValue(listado);
    }
  }
  ...
  cargarDetalles(): void {
    ...
    // La FK en TSRP se llama lsri (igual que en TSRI de CXC)
    db.asignaValorConCampoPadre(TipoDatos.STRING, 'lsri', 'tabla', listado.tabla, TipoComandosBusqueda.IGUAL);
    criterios.push(db);
  ...
  nuevoDetalle(): void {
    ...
    this.formDetalle.reset({ porcentaje: 0, valor: 0, estado: 1 });
  }

  editarDetalle(detalle: DetalleSriCxp): void {
    ...
    this.formDetalle.patchValue({
      ...
      texto: detalle.texto,
      planCuenta: typeof detalle.planCuenta === 'number'
        ? detalle.planCuenta
        : (detalle.planCuenta as PlanCuenta)?.codigo || null,
  ...
  eliminarDetalle(detalle: DetalleSriCxp): void {
    const dialogRef = this.dialog.open<ConfirmDialogComponent, ConfirmDialogData>(
      ConfirmDialogComponent,
      {
        width: '400px',
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed && detalle.id) {
        this.detalleService.delete(detalle.id).subscribe({
          ...
          error: () => this.mostrarError('Error al eliminar detalle'),
        });
      }
    });
  }
  ...
  getCuentaTexto(detalle: DetalleSriCxp): string {
    if (detalle.planCuenta && typeof detalle.planCuenta === 'object') {
  ...
  private mostrarExito(mensaje: string): void {
    this.snackBar.open(mensaje, 'Cerrar', { duration: 3000, panelClass: ['snackbar-success'] });
  }
